...
import { useUserStats, useDueWords, useLearnedWords } from '@/hooks/useVocab'
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('words')
      .select(`*, user_progress!left(id, learning_tier, mastery_score, last_reviewed_at, next_review_date, total_correct, total_wrong)`)
  ...
  // Review progress: tiers 1 & 2 need review, tier 3 = mastered
  const reviewProgress = stats ? {
    learned: stats.learning || 0,
    advanced: stats.advanced || 0,
    mastered: stats.mastered || 0,
    total: stats.wordsStarted || 0,
  } : null
    ...
    <AppLayout>
      <div className="animate-fade-in space-y-8">
          ...
          {showResults && (
            <div className="card overflow-hidden">
              {searching ? (
                ...
                <div className="px-4 py-4 text-sm text-gray-400">No words found for "{debouncedSearch}"</div>
              ) : (
                <>
                  {searchResults.map(word => <WordRow key={word.id} word={word} />)}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          >
            <div className="w-9 h-9 rounded-lg bg-brand-50 flex items-center justify-center group-hover:bg-brand-100 transition-colors mb-3">
              <GraduationCap size={16} className="text-brand-600" />
            </div>
            <p className="text-sm font-semibold text-gray-900">Learn</p>
            <p className="text-xs text-gray-500 mt-0.5">Study new words</p>
          </button>
          ...
          >
            <div className="flex items-center mb-3">
              ...
              {dueWords?.length > 0 && (
                <span className="ml-auto text-xs font-semibold bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded-full">
                  {dueWords.length}
                </span>
              )}
            </div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {dueWords?.length > 0 ? `${dueWords.length} due today` : 'Spaced repetition'}
            </p>
          </button>

          <button
            onClick={() => navigate('/exam')}
            className={`card p-4 text-left hover:shadow-sm transition-all duration-150 group ${
              canExam ? 'hover:border-purple-200' : 'opacity-60'
            }`}
          >
            <div className="flex items-center mb-3">
              ...
              {canExam && (
                <span className="ml-auto text-xs font-semibold bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded-full">
                  {learnedCount}
                </span>
              )}
            </div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {canExam ? 'Timed MCQ test' : `Need ${20 - learnedCount} more`}
            </p>
          </button>
          ...
          >
            <div className="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center group-hover:bg-gray-200 transition-colors mb-3">
              <BookOpen size={16} className="text-gray-600" />
            </div>
            <p className="text-sm font-semibold text-gray-900">Vocabulary</p>
          ...
          {statsLoading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <StatCard
                icon={Zap}
                label="In Progress"
                value={(stats?.learning || 0) + (stats?.advanced || 0)}
                color="bg-yellow-50 text-yellow-600"
                sub={stats?.advanced ? `${stats.advanced} Advanced` : null}
              />
              <StatCard
                icon={Target}
                label="Accuracy"
        ...
        {/* Review + Exam progress — two stat cards in a row */}
        {reviewProgress && reviewProgress.total > 0 && (
          <div>
            <h2 className="text-sm font-semibold text-gray-700 mb-3">Learning Breakdown</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">

              {/* Review tiers card */}
              <div
                className="card p-5 cursor-pointer hover:border-gray-300 hover:shadow-sm transition-all duration-150"
                onClick={() => navigate('/review')}
              >
                ...
                <div className="space-y-2.5">
                  {[
                    { label: 'Mastered', value: reviewProgress.mastered, total: reviewProgress.total, bar: 'bg-green-500' },
                    { label: 'Advanced', value: reviewProgress.advanced, total: reviewProgress.total, bar: 'bg-blue-400' },
                    { label: 'Learned',  value: reviewProgress.learned,  total: reviewProgress.total, bar: 'bg-orange-300' },
              ...
              {/* Exam readiness card */}
              <div
                className={`card p-5 cursor-pointer hover:shadow-sm transition-all duration-150 ${canExam ? 'hover:border-purple-200' : 'opacity-75'}`}
                onClick={() => navigate('/exam')}
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-gray-900">Exam Readiness</p>
                  </div>
                  {canExam
                    ? <span className="badge bg-green-50 text-green-600 text-xs">Ready</span>
                    : <ChevronRight size={13} className="text-gray-300" />
                  }
                </div>
                <div className="space-y-2.5">
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-gray-500">Words available</span>
                      <span className="text-xs font-semibold text-gray-700">{learnedCount} / 20 min</span>
                    </div>
                    <div className="h-1 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-500 ${canExam ? 'bg-purple-500' : 'bg-gray-300'}`}
                        style={{ width: `${Math.min(100, (learnedCount / 20) * 100)}%` }}
                      />
                    </div>
                  </div>
                  <p className="text-xs text-gray-400">
                    {canExam
                      ? `${learnedCount} words ready — take a timed exam`
                      : `${20 - learnedCount} more words needed to unlock`
                    }
                  </p>
                </div>
              </div>
        ...
        {dueWords?.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/review')} className="text-xs text-brand-600 hover:text-brand-700 font-medium">
                View all →
              </button>
            </div>
            <div className="card divide-y divide-gray-100">
              {dueWords.slice(0, 5).map((word) => (
                <div key={word.id} className="px-4 py-3 flex items-center justify-between">
                  ...
                  <span className={`badge text-xs ${getTierInfo(word.progress?.learning_tier ?? 1).color}`}>
                    {getTierInfo(word.progress?.learning_tier ?? 1).label}
                  </span>
                </div>
              ))}
            </div>
